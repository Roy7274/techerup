...
export default function ChatWidget({ visible, onClose, welcomeMessage, inline = false, merchantName, merchantLogo }: ChatWidgetProps) {
    ...
    (async () => {
      try {
        ...
        if (city && city !== '未知') {
          ...
          console.log('已在页面加载时保存城市到会话:', city)
        } else {
  ...
  const ChatContent = (
    <>
      {/* 消息列表 - 移动端优化 */}
      <div className={`chat-messages-container space-y-2.5 sm:space-y-3 py-3 sm:py-4 px-1 sm:px-2 md:px-16 lg:px-24 xl:px-32 2xl:px-40 bg-gray-100 ${inline ? 'pb-24' : 'pb-4'}`} style={{ flex: 1 }}>
        {/* 正在为您服务提示 - 显示在消息列表顶部 */}
          ...
          <div key={msg.id}>
            ...
            >
              ...
              <div className="max-w-[85%] sm:max-w-[70%] md:max-w-[50%] lg:max-w-[40%] xl:max-w-[35%] 2xl:max-w-[30%]">
                <div
                  className={`px-3 py-2.5 sm:px-4 sm:py-3 rounded-2xl text-sm sm:text-base leading-relaxed ${
                    msg.sender === 'user'
        ...
        {loading && (
          <div className="flex justify-start message-bubble">
            ...
            <div className="max-w-[85%] sm:max-w-[70%] md:max-w-[50%] lg:max-w-[40%] xl:max-w-[35%] 2xl:max-w-[30%]">
              <div className="px-3 py-2.5 sm:px-4 sm:py-3 rounded-2xl rounded-tl-sm bg-white shadow-md border border-gray-100">
